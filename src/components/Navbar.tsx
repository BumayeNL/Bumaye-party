...
export const Navbar = ({ logoUrl }: { logoUrl?: string }) => {
  ...
  return (
    <nav className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${scrolled ? 'bg-bumaye-black/90 backdrop-blur-xl py-4 border-b border-white/5' : 'bg-transparent py-6'}`}>
      <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
        ...
        <div className="hidden md:flex items-center gap-8 font-display text-sm uppercase tracking-[0.15em]">
          {['Events','About','Gallery','Contact'].map(item => (
            <a key={item} href={`#${item.toLowerCase()}`} className="hover:text-bumaye-orange transition-colors">{item}</a>
          ))}
          ...
          <button className="bg-bumaye-orange text-white px-8 py-2.5 rounded-full font-bold hover:scale-105 transition-transform shadow-lg shadow-bumaye-orange/20">TICKETS</button>
        </div>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-bumaye-black/98 backdrop-blur-3xl z-[100] flex flex-col items-center justify-center text-center p-6 sm:p-10">
            <button className="absolute top-8 right-8 text-white/40 hover:text-bumaye-orange transition-colors" onClick={() => setIsOpen(false)}><X size={40} /></button>
            <div className="flex flex-col gap-6 font-display text-4xl sm:text-6xl uppercase tracking-tighter">
              {['Events','About','Gallery','Contact'].map((item, i) => (
                <motion.a key={item} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} href={`#${item.toLowerCase()}`} onClick={() => setIsOpen(false)} className="hover:text-bumaye-orange transition-colors">{item}</motion.a>
              ))}
            </div>
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="flex flex-col items-center gap-8 mt-12 w-full max-w-xs">
              <button className="w-full bg-bumaye-orange text-white py-5 rounded-3xl font-bold text-xl hover:scale-105 transition-transform shadow-2xl shadow-bumaye-orange/40">TICKETS KOPEN</button>
