...
export const Hero = ({ gallery, firstEvent }: { gallery: GalleryItem[], firstEvent?: Event }) => (
  <section className="relative min-h-[95vh] md:min-h-screen flex flex-col items-center justify-start md:justify-center pt-28 md:pt-32 pb-20 bg-bumaye-black">
    <div className="max-w-7xl mx-auto px-4 md:px-6 relative z-10 text-center">
      ...
      {gallery && gallery.length > 0 && (
        <div className="w-full max-w-7xl mx-auto px-4 md:px-6 mt-12 md:mt-40 relative z-10">
          <div className="w-full text-left">
            ...
            <div className="flex overflow-x-auto gap-6 hide-scrollbar snap-x snap-mandatory pb-4">
              {gallery.slice(0, 12).map((item, i) => (
                <motion.div key={item.id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.8 + (i * 0.1) }} whileHover={{ scale: 1.05 }} className="flex-none w-[180px] md:w-[240px] aspect-[3/4] rounded-[2rem] overflow-hidden bg-white/5 border border-white/10 snap-start">
