      ...
      <AnimatePresence>
        {showFlyer && event.flyerUrl && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[110] flex items-center justify-center p-4 md:p-10">
            ...
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} className="relative w-full max-w-6xl aspect-[21/9] bg-bumaye-black rounded-[3rem] overflow-hidden shadow-2xl border border-white/10">
              <button onClick={() => setShowFlyer(false)} className="absolute top-8 right-8 z-20 p-3 bg-black/50 hover:bg-bumaye-orange rounded-full transition-colors text-white"><X size={32} /></button>
              <img src={event.flyerUrl} alt={`${event.title} Flyer`} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
